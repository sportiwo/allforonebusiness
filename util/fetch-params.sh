...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/AllForOneBusinessParams"
    else
        PARAMS_DIR="$HOME/.allforonebusiness-params"
    fi
fi
...
Failed to fetch the AllForOneBusiness zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
AllForOneBusiness - fetch-params.sh

This script will fetch the AllForOneBusiness zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common AllForOneBusiness zkSNARK parameters. Note that it is

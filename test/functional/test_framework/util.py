...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s AFO too low! (Should be %s AFO)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 20) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s AFO too high! (Should be %s AFO)" % (str(fee), str(target_fee)))
...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "allforonebusiness.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def get_auth_cookie(datadir):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "allforonebusiness.conf")):
        with open(os.path.join(datadir, "allforonebusiness.conf"), 'r', encoding='utf8') as f:
            for line in f:
...
### AllForOneBusiness specific utils ###

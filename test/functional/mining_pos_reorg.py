...
class ReorgStakeTest(AllForOneBusinessTestFramework):
    ...
    def check_money_supply(self, expected_afo, expected_zafo):
        # verify that nodes have the expected AFO and zAFO supply
        afo_supply = [self.nodes[i].getsupplyinfo(True)['supply']
                      for i in range(self.num_nodes)]
        assert_equal(afo_supply, [DecimalAmt(expected_afo)] * self.num_nodes)
        zafo_supply = [self.nodes[i].getinfo()['zAFOsupply']
                       for i in range(self.num_nodes)]
        for s in zafo_supply:
            for denom in s:
                assert_equal(s[denom], DecimalAmt(expected_zafo[denom]))
    ...
    def run_test(self):
        ...
        # Check AFO and zAFO supply at the beginning
        # ------------------------------------------
        # zAFO supply: 2 coins for each denomination
        expected_zafo_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # AFO supply: block rewards minus burned fees for minting minus mint values
        expected_money_supply = 250.0 * 330 - 16 * 0.01 - 2 * 6666
        self.check_money_supply(expected_money_supply, expected_zafo_supply)
        ...
        # Verify that AFO and zAFO supplies were properly updated after the spends and reorgs
        self.log.info("Check AFO and zAFO supply...")
        ...
        expected_zafo_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zafo_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zafo_supply["total"] -= minted_amount

AllForOneBusiness Core version *4.2.0* is now available from:  <https://github.com/allforonebusiness-project/allforonebusiness/releases>
...
The chainstate database for this release is not compatible with previous releases, so if you run 4.2.0 and then decide to switch back to any older version, you will need to run the old release with the -reindex option to rebuild the chainstate data structures in the old format.
...
Version 4.2.0 contains a number of significant performance improvements, which make Initial Block Download, startup, transaction and block validation much faster:

- The chainstate database (which is used for tracking UTXOs) has been changed from a per-transaction model to a per-output model ([See PR 1801](https://github.com/AllForOneBusiness-Project/AllForOneBusiness/pull/1801)). Advantages of this model are that it:
...
- LevelDB has been upgraded to version 1.22 ([See PR 1738](https://github.com/AllForOneBusiness-Project/AllForOneBusiness/pull/1738)). This version contains hardware acceleration for CRC on architectures supporting SSE 4.2. As a result, synchronization and block validation are now faster.
...
### Miner/Block Generation
 ...
 - #1809 `959d707bc9` [Miner] decouple zAFO duplicated serials checks from CreateNewBlock (furszy)

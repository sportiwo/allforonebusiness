...
*Setup instructions for a gitian build of AllForOneBusiness Core using a VM or physical system.*
...
to the AllForOneBusiness GitHub release page.
...
- [Building AllForOneBusiness Core](#building-allforonebusiness-core)
...
Building AllForOneBusiness Core
--------------------
...
```bash
gpg --output ${VERSION}-linux/${NAME}/allforonebusiness-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/allforonebusiness-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/allforonebusiness-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/allforonebusiness-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/allforonebusiness-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/allforonebusiness-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/allforonebusiness-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@allforonebusiness.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/allforonebusiness-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/allforonebusiness-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/allforonebusiness-osx-*-build.assert
```

...
#include "test/test_allforonebusiness.h"
...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zafoTracker = std::unique_ptr<CzAFOTracker>(new CzAFOTracker(cWallet.strWalletFile));
...
BOOST_AUTO_TEST_CASE(zerocoin_public_spend_test)
{
    ...
    CTxIn in;
    if (!ZAFOModule::createInput(in, mint, tx.GetHash())){
        BOOST_CHECK_MESSAGE(false, "Failed to create zc input");
    }

    PublicCoinSpend publicSpend(ZCParams);
    if (!ZAFOModule::validateInput(in, out, tx, publicSpend)){
        BOOST_CHECK_MESSAGE(false, "Failed to validate zc input");
    }

    PublicCoinSpend publicSpendTest(ZCParams);
    BOOST_CHECK_MESSAGE(ZAFOModule::parseCoinSpend(in, tx, out, publicSpendTest), "Failed to parse public spend");
    ...
    BOOST_CHECK_MESSAGE(!ZAFOModule::validateInput(in, out, tx, publicSpend2), "Different denomination");

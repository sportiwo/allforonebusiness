...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Unix: ~/.allforonebusiness-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AllForOneBusinessParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "AllForOneBusinessParams";
#else
    // Unix
    return pathRet / ".allforonebusiness-params";
#endif
#endif
}
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", AllForOneBusiness_CONF_FILENAME));
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", AllForOneBusiness_PID_FILENAME));

...
// AllForOneBusiness only features
// Masternode
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "allforonebusiness" is a composite category enabling all AllForOneBusiness-related debug output
            if (ptrCategory->count(std::string("allforonebusiness"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\AllForOneBusiness
// Windows >= Vista: C:\Users\Username\AppData\Roaming\AllForOneBusiness
// Mac: ~/Library/Application Support/AllForOneBusiness
// Unix: ~/.allforonebusiness
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AllForOneBusiness";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "allforonebusiness.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty allforonebusiness.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override allforonebusiness.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "allforonebusinessd.pid"));

...
/** Fees smaller than this (in uafo) are considered zero fee (for relaying, mining and transaction creation)
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zAFO mint output in tx %s",
...
CAmount GetBlockValue(int nHeight)
{
    ...
    if (nHeight > nLast)   return 1.5    * COIN;
    if (nHeight > 120000)  return 11   * COIN;
    if (nHeight > 1000)    return 45  * COIN;
...
{
    const int nLast = Params().GetConsensus().vUpgrades[Consensus::UPGRADE_V4_0].nActivationHeight;
    if (nHeight > nLast)   return 0.75 * blockValue;
...
DisconnectResult DisconnectBlock(CBlock& block, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    if (!UpdateZAFOSupplyDisconnect(block, pindex)) {
        error("%s: Failed to calculate new zAFO supply", __func__);
        return DISCONNECT_FAILED;
    }
...
{
    util::ThreadRename("allforonebusiness-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (!UpdateZAFOSupplyConnect(block, pindex, fJustCheck)) {
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        ...
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nAllForOneBusinessBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nAllForOneBusinessBadBlockBits)) {
    ...
    if (isPoS) {
        ...
        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                ...
                afoInputs.push_back(stakeIn);
            }
        }
        const bool hasAFOInputs = !afoInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasAFOInputs) {
                    ...
                    for (const CTxIn& afoIn : afoInputs)
                        if(afoIn.prevout == in.prevout)
...
int ActiveProtocol()
{
    // SPORK_14 is used for 70918 (v4.1.2)
    // if (sporkManager.IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
    //        return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

    // SPORK_15 was used for 70919 (v4.2.0), commented out now.

...
CzAFOWallet::CzAFOWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zafo seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZAFOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZAFOSeed_deprecated()) {
                    LogPrintf("%s: Updated zAFO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zafo seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zafo wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzAFOWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZAFOCount(nCountLastUsed);
    else if (!walletdb.ReadZAFOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzAFOWallet::Lock()
{
    seedMaster = 0;
}

void CzAFOWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzAFOWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZAFO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzAFOWallet::LoadMintPoolFromDB()
{
...
void CzAFOWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzAFOWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzAFOWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zafoTracker->HasPubcoinHash(pMint.first)) {
...
bool CzAFOWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZAFO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zafoTracker which also adds to database
    pwalletMain->zafoTracker->Add(dMint, true);
...
void CzAFOWallet::SeedToZAFO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzAFOWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzAFOWallet::GenerateDeterministicZAFO(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzAFOWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzAFOWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzAFOWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());

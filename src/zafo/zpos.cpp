...
bool CLegacyZAfoStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZAfoStake from non zc-spend", __func__);
    ...
    *this = CLegacyZAfoStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zafo stake origin", __func__);

    // All good
    return true;
}

CLegacyZAfoStake::CLegacyZAfoStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZAfoStake::GetIndexFrom() const
{
...
CAmount CLegacyZAfoStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZAfoStake::GetUniqueness() const
{
...
bool CLegacyZAfoStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zAFO stake block: height %d outside range", __func__, nHeight);

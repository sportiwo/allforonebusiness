...
CzAFOTracker::CzAFOTracker(std::string strWalletFile)
{
...
CzAFOTracker::~CzAFOTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzAFOTracker::Init()
{
...
bool CzAFOTracker::Archive(CMintMeta& meta)
{
...
bool CzAFOTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzAFOTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzAFOTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzAFOTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzAFOTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzAFOTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzAFOTracker::GetSerialHashes()
{
...
CAmount CzAFOTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzAFOTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzAFOTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzAFOTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzAFOTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzAFOTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzAFOTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzAFOTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzAFOTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzAFOTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzAFOTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzAFOWallet* zAFOWallet)
{
    bool iszAFOWalletInitialized = (NULL != zAFOWallet);
    CMintMeta meta;
    ...
    if (! iszAFOWalletInitialized)
        zAFOWallet = new CzAFOWallet(strWalletFile);
    meta.isSeedCorrect = zAFOWallet->CheckSeed(dMint);
    if (! iszAFOWalletInitialized)
        delete zAFOWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzAFOTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzAFOTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzAFOTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzAFOTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzAFOTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzAFOTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzAFOWallet* zAFOWallet = new CzAFOWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zAFOWallet);
        }
        delete zAFOWallet;
        LogPrint("zero", "%s: added %d dzafo from DB\n", __func__, listDeterministicDB.size());
    }

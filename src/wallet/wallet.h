...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zafospends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT1000IFMN = 3,
    ONLY_NONDENOMINATED_NOT1000IFMN = 4, // ONLY_NONDENOMINATED and not 1000 AFO at the same time
    ONLY_1000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zAFO send
enum ZerocoinSpendStatus {
    ZAFO_SPEND_OKAY = 0,                            // No error
    ZAFO_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZAFO_WALLET_LOCKED = 2,                         // Wallet was locked
    ZAFO_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZAFO_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZAFO_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZAFO_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZAFO_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZAFO_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZAFO_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZAFO_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZAFO_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZAFO_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZAFO_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZAFO_SPENT_USED_ZAFO = 14,                      // Coin has already been spend
    ZAFO_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZAFO_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZafoBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZAFOOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzafoAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzAFOTracker> zafoTracker;
    ...
    void setZWallet(CzAFOWallet* zwallet)
    {
        zwalletMain = zwallet;
        zafoTracker = std::unique_ptr<CzAFOTracker>(new CzAFOTracker(strWalletFile));
    }

    CzAFOWallet* getZWallet() { return zwalletMain; }
    ...
    void setZafoAutoBackups(bool fEnabled)
    {
    ...
    /** zAFO reset */
    boost::signals2::signal<void()> NotifyzAFOReset;

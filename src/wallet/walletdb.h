...
public:
    ...
    bool WriteZAFOSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZAFOSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZAFOSeed_deprecated(uint256& seed);
    bool EraseZAFOSeed();
    bool EraseZAFOSeed_deprecated();

    bool WriteZAFOCount(const uint32_t& nCount);
    bool ReadZAFOCount(uint32_t& nCount);

...
/**
 * Fees smaller than this (in uafo) are considered zero fee (for transaction creation)
        ...
        if (zwallet) {
            ...
            if (CWalletDB(strWalletFile).ReadCurrentSeedHash(hashSeed)) {
                ...
                if (!GetDeterministicSeed(hashSeed, nSeed)) {
                    return error("Failed to read zAFO seed from DB. Wallet is probably corrupt.");
                }
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    bool fCheckZAFO = gArgs.GetBoolArg("-zapwallettxes", false);
    if (fCheckZAFO)
        zafoTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            doZAfoRescan(pindex, block, setAddedToWallet, consensus, fCheckZAFO);
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex, std::string& strError)
{
    ...
    if (txOut.nValue != 1000 * COIN) {
        strError = "Invalid collateral tx value, must be 10,000 AFO";
        ...
        std::vector<CStakeableOutput>* availableCoins)
{
    ...
    for (const auto& out : *availableCoins) {
        CAfoStake stakeInput(out.tx->vout[out.i],
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in AFO) allowed by GUI and RPC for the stake split threshold (default: %s)"), FormatMoney(DEFAULT_MIN_STAKE_SPLIT_THRESHOLD)));
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (nLoadWalletRet != DB_LOAD_OK) {
        if (nLoadWalletRet == DB_CORRUPT) {
            ...
            UIError(strprintf(_("Error loading %s: Wallet requires newer version of AllForOneBusiness Core"), walletFile));
            return nullptr;
        } else if (nLoadWalletRet == DB_NEED_REWRITE) {
            UIError(_("Wallet needed to be rewritten: restart AllForOneBusiness Core to complete"));
            return nullptr;
        } else {
    ...
    if (gArgs.GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet)) {
        if (prev_version <= FEATURE_PRE_AllForOneBusiness && walletInstance->IsLocked()) {
    ...
    if (fFirstRun) {
        if (!fLegacyWallet) {
            ...
            walletInstance->SetMaxVersion(FEATURE_PRE_AllForOneBusiness);
        }
    ...
    CzAFOWallet* zwalletInstance = new CzAFOWallet(walletInstance);
    ...
    if (!zwalletInstance->GetMasterSeed().IsNull()) {
        ...
        walletInstance->zafoTracker->Init();

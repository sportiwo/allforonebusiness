    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteZAFOSeed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteZAFOSeed(hashSeed, ToByteVector(seed))) {
            return true;
        }
        strErr = "save zafoseed to wallet";
    }
...
bool CWallet::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            //read encrypted seed
            if (db.ReadZAFOSeed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadZAFOSeed(hashSeed, vchSeed)) {
...
void CWallet::doZAfoRescan(const CBlockIndex* pindex, const CBlock& block,
        std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZAFO)
{
    //If this is a zapwallettx, need to read zafo
    if (fCheckZAFO && consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC)) {
...
bool CWallet::CreateZAFOOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwallet->GenerateDeterministicZAFO(denomination, coin, dMint);
        ...
        std::list<std::pair<CTxDestination, CAmount>> addressesTo, CTxDestination* changeAddress)
{
    ...
    receipt.SetStatus("Spend Successful", ZAFO_SPEND_OKAY);  // When we reach this point spending zPIV was successful
                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    for (auto &it : mapMintsSelected) {
        ...
        CTxIn in;
        if(!ZAFOModule::createInput(in, mint, hashTxOut, spendVersion)) {
...
void CWallet::setZWallet(CzAFOWallet* zwallet)
{
    this->zwallet = zwallet;
    zafoTracker = std::unique_ptr<CzAFOTracker>(new CzAFOTracker(this));
}

CzAFOWallet* CWallet::getZWallet()
{
    return zwallet;
}

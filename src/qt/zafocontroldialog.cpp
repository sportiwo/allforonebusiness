...
#include "ui_zafocontroldialog.h"
...
std::set<std::string> ZafoControlDialog::setSelectedMints;
std::set<CMintMeta> ZafoControlDialog::setMints;

bool CZafoControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZafoControlDialog::COLUMN_DENOMINATION || column == ZafoControlDialog::COLUMN_VERSION || column == ZafoControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZafoControlDialog::ZafoControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZafoControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZafoControlDialog::~ZafoControlDialog()
{
    delete ui;
}

void ZafoControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZafoControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZafoControlWidgetItem* itemDenom(new CZafoControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZafoControlWidgetItem *itemMint = new CZafoControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zafoTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zafoTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zAFO.";
            else if (!mint.isSeedCorrect)
                strReason = "The zAFO seed used to mint this zAFO is not the same as currently hold in the wallet";
            else
...
void ZafoControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZafoControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZafo_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZafoControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZafoControlDialog::GetSelectedMints()
{
...
void ZafoControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

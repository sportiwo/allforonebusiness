                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zAFO ought to be enough for anybody." - Bill Gates, 2017
    ui->zAFOpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzAFOSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zAFOpayAmount->setFocus();
    }
}
/* disable MINT
 *
void PrivacyDialog::on_pushButtonMintzAFO_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zAFO is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zAFO, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zAFO...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zAFO in ") +
...
void PrivacyDialog::on_pushButtonSpendzAFO_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zAFO is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zAFO, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zAFO
        sendzAFO();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zAFO
    sendzAFO();
}

void PrivacyDialog::on_pushButtonZafoControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZafoControlDialog* ZafoControl = new ZafoControlDialog(this);
    ZafoControl->setModel(walletModel);
    ZafoControl->exec();
}

void PrivacyDialog::setZafoControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelZafoSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzAFO()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid AllForOneBusiness Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zAFOpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zAFO is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " AFO </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zAFO</b>";
    ...
    // use mints from zAFO selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZafoControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZafoControlDialog::GetSelectedMints();
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zAFO transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zAFO was spent successfully update the addressbook with the label
    ...
    // Clear zafo selector in case it was used
    ZafoControlDialog::setSelectedMints.clear();
    ui->labelZafoSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zAFO Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zafoTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zAFO </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zAFO "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zAFO "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zAFO "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zAFO </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zAFO </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zAFO </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzAFOSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzAFO->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zAFO
        //ui->pushButtonMintzAFO->setEnabled(false);
        //ui->pushButtonMintzAFO->setToolTip(tr("zAFO is currently disabled due to maintenance."));

        // Spend zAFO
        ui->pushButtonSpendzAFO->setEnabled(false);
        ui->pushButtonSpendzAFO->setToolTip(tr("zAFO is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zAFO
        //ui->pushButtonMintzAFO->setEnabled(true);
        //ui->pushButtonMintzAFO->setToolTip(tr("PrivacyDialog", "Enter an amount of AFO to convert to zAFO", 0));

        // Spend zAFO
        ui->pushButtonSpendzAFO->setEnabled(true);
        ui->pushButtonSpendzAFO->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}

...
#include "qt/allforonebusiness/forms/ui_addresseswidget.h"
#include "qt/allforonebusiness/addresslabelrow.h"
#include "qt/allforonebusiness/addnewaddressdialog.h"
#include "qt/allforonebusiness/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(AllForOneBusinessGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto afoAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!IsValidDestination(afoAdd) || isStakingAddress) {
        ...
        if (walletModel->updateAddressBookLabels(afoAdd, label.toUtf8().constData(),

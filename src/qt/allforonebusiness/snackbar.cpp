...
#include "qt/allforonebusiness/forms/ui_snackbar.h"
#include "qt/allforonebusiness/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(AllForOneBusinessGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &AllForOneBusinessGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {

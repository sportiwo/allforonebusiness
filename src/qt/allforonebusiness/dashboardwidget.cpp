...
#include "qt/allforonebusiness/dashboardwidget.h"
#include "qt/allforonebusiness/forms/ui_dashboardwidget.h"
#include "qt/allforonebusiness/sendconfirmdialog.h"
#include "qt/allforonebusiness/txrow.h"
...
DashboardWidget::DashboardWidget(AllForOneBusinessGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareAfo, "square-chart-afo");
    setCssProperty(ui->labelSquarezAfo, "square-chart-zafo");
    setCssProperty(ui->labelAfo, "text-chart-afo");
    setCssProperty(ui->labelZafo, "text-chart-zafo");
    ui->labelZafo->setVisible(false);
    ui->labelSquarezAfo->setVisible(false);
    ...
    setCssProperty(ui->labelAmountAfo, "text-stake-afo-disable");
    setCssProperty(ui->labelAmountZafo, "text-stake-zafo-disable");
    ui->labelAmountZafo->setVisible(false);
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &AllForOneBusinessGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isAfo = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZAFO;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            afo = (pair.first != 0) ? pair.first / 100000000 : 0;
            zafo = (pair.second != 0) ? pair.second / 100000000 : 0;
        ...
        chartData->valuesAfo.append(afo);
        chartData->valueszAfo.append(zafo);

        int max = std::max(afo, zafo);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set1->append(chartData->valueszAfo);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalAfo > 0 || chartData->totalZafo > 0) {
        setCssProperty(ui->labelAmountAfo, "text-stake-afo");
        setCssProperty(ui->labelAmountZafo, "text-stake-zafo");
    } else {
        setCssProperty(ui->labelAmountAfo, "text-stake-afo-disable");
        setCssProperty(ui->labelAmountZafo, "text-stake-zafo-disable");
    }
    forceUpdateStyle({ui->labelAmountAfo, ui->labelAmountZafo});
    ui->labelAmountAfo->setText(GUIUtil::formatBalance(chartData->totalAfo, nDisplayUnit));
    ui->labelAmountZafo->setText(GUIUtil::formatBalance(chartData->totalZafo, nDisplayUnit, true));

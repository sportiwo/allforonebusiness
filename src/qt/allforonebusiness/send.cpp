...
#include "qt/allforonebusiness/forms/ui_send.h"
#include "qt/allforonebusiness/addnewcontactdialog.h"
#include "qt/allforonebusiness/qtutils.h"
#include "qt/allforonebusiness/sendchangeaddressdialog.h"
#include "qt/allforonebusiness/optionbutton.h"
#include "qt/allforonebusiness/sendconfirmdialog.h"
#include "qt/allforonebusiness/myaddressrow.h"
#include "qt/allforonebusiness/guitransactionsutils.h"
...
SendWidget::SendWidget(AllForOneBusinessGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-afo");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto afoAdd = DecodeDestination(address.toStdString(), isStakingAddr);

        if (!IsValidDestination(afoAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(afoAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(afoAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {

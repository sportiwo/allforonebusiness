...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    ...
    if (settings.contains("nAnonymizeAllForOneBusinessAmount") || reset)
        gArgs.SoftSetArg("-anonymizeallforonebusinessamount", settings.value("nAnonymizeAllForOneBusinessAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in allforonebusiness.cpp during shotdown to also remove the window positions

...
CZafoStake::CZafoStake(const libzerocoin::CoinSpend& spend)
{
...
int CZafoStake::GetChecksumHeightFromMint()
{
...
int CZafoStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZafoStake::GetChecksum()
{
    return nChecksum;
}

// The zAFO block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZafoStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZafoStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CZafoStake::GetUniqueness()
{
    //The unique identifier for a zAFO is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZafoStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZafoStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zAFO that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZAFOOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zAFO output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zAFO", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZAFOOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zAFO output", __func__);
...
bool CZafoStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZafoStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzAFOTracker* zafoTracker = pwallet->zafoTracker.get();
    CMintMeta meta;
    if (!zafoTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zafoTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a AFO stake is the outpoint

...
CAfoStake* CAfoStake::NewAfoStake(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize CAfoStake from zerocoin spend", __func__);
        return nullptr;
    }
    ...
    return new CAfoStake(txPrev.vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CAfoStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CAfoStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CAfoStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CAfoStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CAfoStake::GetUniqueness() const
{
...
const CBlockIndex* CAfoStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CAfoStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CAfoStake::ContextCheck(int nHeight, uint32_t nTime)
{

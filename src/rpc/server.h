...
/**
 * AllForOneBusiness RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZafoSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzafoseed(const UniValue& params, bool fHelp);
extern UniValue getzafoseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzafo(const UniValue& params, bool fHelp);
extern UniValue dzafostate(const UniValue& params, bool fHelp);

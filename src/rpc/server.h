...
extern UniValue DoZafoSpend(const CAmount nAmount, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str);
...
extern UniValue setzafoseed(const JSONRPCRequest& request);
extern UniValue getzafoseed(const JSONRPCRequest& request);
extern UniValue generatemintlist(const JSONRPCRequest& request);
extern UniValue searchdzafo(const JSONRPCRequest& request);
extern UniValue dzafostate(const JSONRPCRequest& request);

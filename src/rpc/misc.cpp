...
UniValue getinfo(const JSONRPCRequest& request)
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        if (mapZerocoinSupply.empty())
            zafoObj.pushKV(std::to_string(denom), ValueFromAmount(0));
        else
            zafoObj.pushKV(std::to_string(denom), ValueFromAmount(mapZerocoinSupply.at(denom) * (denom*COIN)));
    }
    zafoObj.pushKV("total", ValueFromAmount(GetZerocoinSupply()));
//    obj.pushKV("zAFOsupply", zafoObj);
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for AllForOneBusiness cold staking -only if is standard address-\n"
...
UniValue getstakingstatus(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"staking_enabled\": true|false,     (boolean) whether staking is enabled/disabled in allforonebusiness.conf\n"
            "  \"coldstaking_enabled\": true|false, (boolean) whether cold-staking is enabled/disabled in allforonebusiness.conf\n"
            "  \"haveconnections\": true|false,     (boolean) whether network connections are present\n"
            "  \"mnsync\": true|false,              (boolean) whether the required masternode/spork data is synced\n"
            "  \"walletunlocked\": true|false,      (boolean) whether the wallet is unlocked\n"
            "  \"stakeablecoins\": n                (numeric) number of stakeable UTXOs\n"
            "  \"stakingbalance\": d                (numeric) AFO value of the stakeable coins (minus reserve balance, if any)\n"

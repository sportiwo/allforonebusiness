...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total allforonebusiness balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zAFOsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zAFO denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zAFO denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zAFO denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zAFO denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zAFO denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zAFO denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zAFO denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zAFO denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zAFO denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in allforonebusiness/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in allforonebusiness/kb\n"
    ...
    UniValue zafoObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zafoObj.push_back(Pair(std::to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zafoObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("zAFOsupply", zafoObj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"allforonebusinessaddress\"\n"
            "\nReturn information about the given allforonebusiness address.\n"

            "\nArguments:\n"
            "1. \"allforonebusinessaddress\"     (string, required) The allforonebusiness address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"allforonebusinessaddress\", (string) The allforonebusiness address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: AllForOneBusiness address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are allforonebusiness addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) allforonebusiness address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"allforonebusinessaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"allforonebusinessaddress\"  (string, required) The allforonebusiness address to use for the signature.\n"
